...
('OOP','Basics','Easy','What is Object Oriented Programming?',
'Object Oriented Programming OOP is a programming paradigm that organizes software design around objects rather than functions and logic. An object is an instance of a class that contains data in the form of fields called attributes and code in the form of procedures called methods. OOP is based on four main principles: Encapsulation, Inheritance, Polymorphism, and Abstraction. Examples of OOP languages include Java Python C++ and C#. OOP makes code more modular reusable and easier to maintain compared to procedural programming.'),

('OOP','Basics','Easy','What is a class and an object?',
'A class is a blueprint or template that defines the properties and behaviors that objects of that type will have. It defines attributes which are data fields and methods which are functions. An object is an instance of a class — a concrete realization of the blueprint with actual values. For example Car is a class with attributes like color speed and brand and methods like accelerate and brake. A specific car like a red Toyota is an object — an instance of the Car class. Multiple objects can be created from the same class each with their own attribute values.'),

('OOP','Basics','Easy','What is encapsulation?',
'Encapsulation is the OOP principle of bundling data attributes and methods that operate on that data together within a class and restricting direct access to some components. It hides internal implementation details from the outside world. Access is controlled through access modifiers: private members are accessible only within the class, protected within the class and subclasses, and public accessible from anywhere. Getters and setters provide controlled access to private attributes. Benefits include data hiding preventing accidental modification, easier maintenance since internal changes do not affect external code, and better modularity.'),

('OOP','Basics','Easy','What is inheritance?',
'Inheritance is the OOP mechanism where a new class called child or subclass acquires the properties and behaviors of an existing class called parent or superclass. The child class inherits all non-private attributes and methods of the parent and can add new ones or override existing ones. It promotes code reusability since common functionality is defined once in the parent. For example Animal class has eat and sleep methods. Dog class inherits from Animal and adds bark method. Types include single inheritance one parent, multiple inheritance multiple parents supported in Python C++, multilevel inheritance chain of inheritance, and hierarchical inheritance multiple children from one parent.'),

('OOP','Basics','Easy','What is polymorphism?',
'Polymorphism means one name many forms. It allows objects of different classes to be treated as objects of a common superclass and the same method call to produce different behaviors based on the actual object type. Compile time polymorphism is achieved through method overloading where multiple methods have the same name but different parameters. Runtime polymorphism is achieved through method overriding where a subclass provides a specific implementation of a method already defined in the parent class. For example a Shape class has a draw method. Circle and Rectangle subclasses override draw to produce different outputs. Polymorphism makes code more flexible and extensible.'),

('OOP','Basics','Easy','What is abstraction?',
'Abstraction is the OOP principle of hiding complex implementation details and showing only the essential features of an object. It focuses on what an object does rather than how it does it. Abstraction is achieved through abstract classes which cannot be instantiated and may contain abstract methods with no implementation that subclasses must implement, and interfaces which define a contract of methods that implementing classes must provide. For example a Database abstract class defines connect query and close methods without implementation. MySQL and PostgreSQL subclasses provide actual implementations. Users interact with the abstract interface without knowing database-specific details.'),

('OOP','Basics','Easy','What is the difference between abstraction and encapsulation?',
'Abstraction hides complexity by showing only relevant features to the user — it is about design and what information to expose. For example a car driver uses the steering wheel and pedals without knowing the engine internals. Encapsulation hides data by bundling it with methods and controlling access through access modifiers — it is about implementation and protecting data integrity. For example a BankAccount class hides the balance variable as private and provides deposit and withdraw methods to modify it safely. Abstraction is achieved through abstract classes and interfaces while encapsulation is achieved through access modifiers and getter setter methods. Both work together to create well-designed OOP systems.'),

('OOP','Basics','Easy','What is a constructor?',
'A constructor is a special method in a class that is automatically called when an object is created. It has the same name as the class and no return type. Its purpose is to initialize the object attributes to their initial values. A default constructor takes no parameters and initializes attributes to default values. A parameterized constructor takes arguments to initialize attributes with specific values. In Java and C++ if no constructor is defined the compiler provides a default one. A copy constructor creates a new object as a copy of an existing object. Constructors ensure that objects are always in a valid initialized state when created.'),

('OOP','Basics','Easy','What is method overloading?',
'Method overloading is compile time polymorphism where multiple methods in the same class have the same name but different parameter lists — different number of parameters different types or different order. The compiler determines which method to call based on the arguments provided at compile time. For example a Calculator class can have add(int a, int b) and add(double a, double b) and add(int a, int b, int c) — all named add but with different signatures. Return type alone cannot differentiate overloaded methods. Method overloading improves code readability by using the same intuitive name for related operations.'),

('OOP','Basics','Easy','What is method overriding?',
'Method overriding is runtime polymorphism where a subclass provides its own specific implementation of a method already defined in the parent class. The method in the subclass must have the same name return type and parameters as the parent method. The override is resolved at runtime based on the actual object type not the reference type. For example Animal class has makeSound method that prints generic sound. Dog overrides makeSound to print Woof and Cat overrides it to print Meow. When makeSound is called on an Animal reference pointing to a Dog object it prints Woof. The Override annotation in Java verifies correct overriding at compile time.'),

('OOP','Basics','Easy','What is an interface?',
'An interface in OOP is a completely abstract type that defines a contract — a set of method signatures that implementing classes must provide. Interfaces contain only method declarations with no implementation and constants. A class can implement multiple interfaces providing flexibility that single inheritance does not allow. Interfaces define what a class can do without specifying how. For example Flyable interface has fly method. Both Bird and Airplane classes can implement Flyable and provide their own fly implementations. Interfaces enable loose coupling since code can depend on interfaces rather than concrete classes making it easy to swap implementations.'),

('OOP','Basics','Easy','What is the difference between an abstract class and an interface?',
'Abstract class can have both abstract methods with no body and concrete methods with implementation. It can have instance variables constructors and access modifiers. A class can extend only one abstract class due to single inheritance. Use abstract class when subclasses share common code and state. Interface can only have abstract methods in older Java versions but Java 8 added default and static methods. Interface variables are implicitly public static final constants. A class can implement multiple interfaces. Use interface when unrelated classes need to share a contract or when multiple inheritance of type is needed. Abstract class is for is-a relationship while interface is for can-do relationship.'),

-- ─── OOP MEDIUM ──────────────────────────────────────────────────────────────

('OOP','Basics','Medium','What are the four pillars of OOP and explain each briefly?',
'Encapsulation bundles data and methods together and restricts access using access modifiers protecting data integrity and hiding implementation details. Inheritance allows a child class to acquire properties and behaviors of a parent class promoting code reuse and establishing is-a relationships. Polymorphism allows the same interface to be used for different underlying forms — method overloading at compile time and method overriding at runtime enabling flexible extensible code. Abstraction hides complex implementation and exposes only essential features through abstract classes and interfaces allowing users to interact with high level concepts without knowing internals. These four pillars work together to create maintainable modular and reusable object oriented code.'),

('OOP','Advanced','Medium','What is the difference between composition and inheritance?',
'Inheritance creates an is-a relationship where a subclass is a specialized version of the parent. Dog is an Animal. The subclass inherits and can override parent behavior. Composition creates a has-a relationship where a class contains instances of other classes as members. Car has an Engine. Changes to the parent in inheritance can break subclasses creating tight coupling. Composition provides loose coupling since the containing class depends on interface not implementation. Composition is more flexible than inheritance — you can change the composed object at runtime. The principle composition over inheritance suggests preferring composition when both options are available as it leads to more flexible designs. Use inheritance for true is-a relationships and composition for has-a relationships.'),

('OOP','Advanced','Medium','What is the SOLID principle?',
'SOLID is five design principles for OOP. Single Responsibility Principle states a class should have only one reason to change — one responsibility. Open Closed Principle states classes should be open for extension but closed for modification — add new functionality through inheritance not by changing existing code. Liskov Substitution Principle states objects of a subclass should be replaceable with objects of the parent class without breaking the program. Interface Segregation Principle states clients should not be forced to depend on interfaces they do not use — prefer many specific interfaces over one large general interface. Dependency Inversion Principle states high level modules should depend on abstractions not concrete implementations. SOLID principles lead to maintainable flexible and testable code.'),

('OOP','Advanced','Medium','What is a design pattern? Explain Singleton pattern.',
'A design pattern is a reusable solution to a commonly occurring problem in software design. They are templates for solving problems not finished code. Creational patterns deal with object creation. Structural patterns deal with object composition. Behavioral patterns deal with object interaction. Singleton pattern ensures a class has only one instance and provides a global point of access to it. Implementation: make constructor private, create a static instance variable, provide a static getInstance method that creates the instance on first call and returns existing instance on subsequent calls. Used for database connections configuration objects and logging. Thread safety requires synchronization or double-checked locking in multithreaded environments.'),

('OOP','Advanced','Medium','What is the difference between shallow copy and deep copy?',
'A shallow copy creates a new object but copies only the references to the nested objects not the objects themselves. Both original and copy point to the same nested objects — modifying a nested object through one affects the other. Shallow copy is fast and uses less memory. A deep copy creates a new object and recursively copies all nested objects creating completely independent copies. Modifying one does not affect the other. Deep copy is slower and uses more memory but provides true independence. In Java clone method performs shallow copy by default. Deep copy requires custom implementation or serialization. Example: copying a list of objects — shallow copy gives a new list pointing to same objects while deep copy creates new copies of each object.'),

('OOP','Advanced','Medium','What is the difference between static and instance methods?',
'Instance methods belong to an object instance and can access and modify instance variables. They require an object to be called like obj.method(). They have access to the this reference. Static methods belong to the class itself not to any instance. They are called on the class like ClassName.method(). They cannot access instance variables or instance methods directly since no object context exists. They can only access static variables and other static methods. Static methods are useful for utility functions that do not depend on object state like Math.sqrt() in Java. Static variables are shared across all instances of a class. Instance variables are separate for each object.'),

('OOP','Advanced','Medium','What is exception handling in OOP?',
'Exception handling is a mechanism to handle runtime errors gracefully without crashing the program. An exception is an event that disrupts normal program flow. Try block contains code that might throw an exception. Catch block handles specific exceptions providing recovery logic. Finally block executes always regardless of exception for cleanup like closing resources. Throw keyword manually raises an exception. Throws keyword in method signature declares what exceptions a method might throw. Checked exceptions must be handled or declared like IOException. Unchecked exceptions extend RuntimeException and do not require handling like NullPointerException. Custom exceptions extend Exception class to represent application-specific error conditions. Exception hierarchy allows catching parent types to handle multiple related exceptions.'),

-- ─── OOP HARD ────────────────────────────────────────────────────────────────

('OOP','Advanced','Hard','Explain design patterns — Factory Method and Observer.',
'Factory Method is a creational pattern that defines an interface for creating objects but lets subclasses decide which class to instantiate. Instead of calling new directly you call a factory method. This decouples object creation from usage. Example: ShapeFactory with createShape method returns Circle or Rectangle based on parameter. Subclasses override the factory method to change the type of object created. Observer pattern is a behavioral pattern defining one-to-many dependency where when one object the subject changes state all dependents observers are notified automatically. Example: event listeners in GUI — button is subject multiple handlers are observers. When button is clicked all registered handlers are notified. Observer enables loose coupling between subject and observers.'),

('OOP','Advanced','Hard','What is dependency injection and why is it important?',
'Dependency injection is a design pattern where an object receives its dependencies from outside rather than creating them internally. Instead of a class instantiating its dependencies they are passed in through constructor injection via constructor parameters, setter injection via setter methods, or interface injection. This implements the Dependency Inversion Principle. Benefits include loose coupling since classes depend on abstractions not concrete implementations, testability since dependencies can be mocked in unit tests, flexibility since different implementations can be swapped without changing dependent class code, and single responsibility since classes do not manage their dependency lifecycle. Dependency injection frameworks like Spring for Java handle wiring automatically making large applications more maintainable.'),

('OOP','Advanced','Hard','What is the difference between early binding and late binding?',
'Early binding also called static binding occurs at compile time when the compiler determines which method to call based on the declared type of the reference. Method overloading and calls to private static and final methods use early binding since the compiler can determine the exact method at compile time. It is faster since no runtime lookup is needed. Late binding also called dynamic binding occurs at runtime when the JVM determines which method to call based on the actual type of the object not the declared type. Method overriding uses late binding enabling runtime polymorphism. When you call a method on a parent reference pointing to a child object the child method is called at runtime. Late binding is slightly slower but essential for polymorphism and extensibility.'),

('OOP','Advanced','Hard','Explain the concept of generics in OOP.',
'Generics allow classes interfaces and methods to operate on objects of various types while providing compile time type safety. Instead of working with Object type requiring casting generics use type parameters like T. For example a generic Stack class Stack with T allows creating Stack of Integer or Stack of String — the compiler enforces type safety preventing ClassCastException at runtime. Benefits include type safety catching type errors at compile time not runtime, code reusability writing one class that works for any type, and elimination of casting making code cleaner. Generic methods can have their own type parameters independent of the class. Bounded type parameters like extends Number restrict which types can be used. Java type erasure removes generic type information at runtime for backward compatibility.'),

('OOP','Advanced','Hard','What are SOLID principles and give a real example of each?',
'Single Responsibility: A User class should not handle both user data and email sending. Split into User and EmailService. Open Closed: Instead of modifying a PaymentProcessor class when adding new payment methods extend it — create PayPalPayment and CreditCardPayment implementing a Payment interface. Liskov Substitution: If Bird has fly method and Penguin extends Bird but cannot fly this violates LSP. Solution: separate FlyingBird and NonFlyingBird classes. Interface Segregation: Instead of one fat Worker interface with work eat and sleep methods create separate Workable Eatable and Sleepable interfaces so robots implement only Workable. Dependency Inversion: Instead of OrderService directly creating MySQLDatabase instance inject a Database interface allowing easy switching to PostgreSQL or MongoDB without changing OrderService.');
-- TABLE 3: ANSWERS
